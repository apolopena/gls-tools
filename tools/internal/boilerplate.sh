...
# The code runs as-is and uses the most of the same global long options the other gls-tools use
# This script will download the latest release version of gitpod laravel starter
# to tmp_gls_update/x.x.x where x is an integer
# That tmp_gls_update/x.x.x directory will be created relative to where this script is run from
...
# Temporary working directory for the REPLACE_WITH_SCRIPT_NAME. Set by init()
tmp_dir=

# Location for the download of latest version of gls to REPLACE_WITH_SCRIPT_NAME the project to. Set by set_target_version()
target_dir=

# Location for recommended backups. Set by init(). Appended by REPLACE_WITH_SCRIPT_NAME().
backups_dir=

# Latest release data downloaded from github. Set by init()
release_json=

# The version to REPLACE_WITH_SCRIPT_NAME to. Set by init()
target_version=
...
version() {
  echo "vx.x.x
REPLACE_WITH_SCRIPT_NAME is a gitpod-larvel-starter tool from the gls-tools suite
Copyright © 2022 Apolo Pena
...
name() {
  printf '%s' "${c_file_name}REPLACE_WITH_SCRIPT_NAME.sh${c_e}"
}
...
#
# Note:
# This function can only be called once and prior to calling REPLACE_WITH_SCRIPT_NAME()
...
init() {
  ...
  project_root="$(pwd)"
  backups_dir="$project_root"; # Will be mutated intentionally by REPLACE_WITH_SCRIPT_NAME()
  ...
  # Success. It is now safe to call REPLACE_WITH_SCRIPT_NAME()
...
REPLACE_WITH_SCRIPT_NAME() {
  ...
  e_fail_prefix="${c_norm_prob}REPLACE_WITH_SCRIPT_NAME() internal error: Failed to"
...
#   5. init() --> REPLACE_WITH_SCRIPT_NAME() --> cleanup()
#
# Note:
...
main() {
  ...
  # Initialize, REPLACE_WITH_SCRIPT_NAME and finally cleanup
